...
`harness/latency.mjs` 를 트리당 5회씩 돌린 중앙값 기준 **41,288ms → 3,196ms**
(v9.5.0 은 39,199–42,864 로 일정하고, 9.5.1 은 125–3,566 으로 편차가 큽니다).
홈 화면도 v9.5.0 에서는 5회 중 2회가 19초대였습니다 — Lean 만의 문제가 아니었습니다.

**이 지표는 1회 측정으로 판단하지 마세요.** 훑기 비용이 그때까지 쌓인 DOM 양에
비례해서 실행마다 어느 화면이 걸릴지가 달라집니다. 처음 잰 39,761ms 도, 나중에 잰
211ms 도 각각 한 번의 뽑기였습니다. 최소 5회 재고 중앙값과 범위를 함께 보세요.
